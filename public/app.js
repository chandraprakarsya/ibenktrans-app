const BASE_URL = window.location.origin;
    ...
    async function openModalTransaksi() {
      try {
        ...
        listMobil.forEach(m => {
          ...
          const tag = isInv ? '[INVESTOR]' : '[MILIK IBENK TRANS]';
    ...
    function renderCharts(details, summary, trendHarian) {
      ...
      // Doughnut Chart Bagi Hasil (Porsi Ibenk Trans vs Hak Mitra Investor)
      ...
      if (ctxPie) {
        pieChart = new Chart(ctxPie, {
          type: 'doughnut',
          data: {
            labels: ['Porsi Ibenk Trans (30% + Unit Sendiri)', 'Hak Mitra Investor (70% - Ops)'],
            datasets: [{
    ...
    async function openEditMobil(id) {
      try {
        ...
        document.getElementById('edit-armada-kepemilikan').value = (mobil.kepemilikan && mobil.kepemilikan.toLowerCase().includes('investor')) ? 'Investor' : 'Ibenk Trans';
        ...
        let html = `
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div class="bg-blue-50 border border-blue-200 p-3.5 rounded-xl shadow-xs">
              <p class="text-[10px] font-bold uppercase tracking-wider text-blue-700">Hak Pengelola Ibenk Trans</p>
          ...
          html += `
              ...
              <div class="bg-gray-50/90 p-4 border-t border-gray-200">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-3 text-xs">
                  ...
                  <div class="bg-white p-2.5 rounded-lg border border-gray-200">
                    <span class="text-gray-500 font-semibold block text-[10px] uppercase">Porsi Ibenk Trans (30% Gross)</span>
    ...
    function downloadPDFReport() {
      ...
      const opt = {
        margin:       0.5,
        filename:     `Laporan_Investor_IbenkTrans_${new Date().toISOString().split('T')[0]}.pdf`,
    ...
    async function populateAllReportMobilDropdown() {
      try {
        ...
        if (currentAllReportCategory === 'investor') {
          ...
          filteredCars = filteredCars.filter(m => !(m.kepemilikan && m.kepemilikan.toLowerCase().includes('investor')));
          defaultLabel = '🚗 Semua Unit Milik Ibenk Trans';
        }
    ...
    async function loadAllReport() {
      ...
      let catLabel = currentAllReportCategory === 'investor' 
        ? 'Unit Investor (70:30)' 
        : (currentAllReportCategory === 'sendiri' ? 'Unit Milik Ibenk Trans' : 'Semua Unit');
      ...
      if (subTitleEl) {
        if (currentAllReportCategory === 'investor') {
          subTitleEl.innerText = 'Laporan Khusus Pengelolaan Unit Armada Investor (Skema 70% Investor : 30% Ibenk Trans)';
        } else if (currentAllReportCategory === 'sendiri') {
          subTitleEl.innerText = 'Laporan Khusus Pengelolaan Unit Milik Sendiri Ibenk Trans (100% Kas Bersih)';
        } else {
      ...
      if (currentAllReportCategory === 'investor') {
        ...
        if (lblPeng) lblPeng.innerText = 'Porsi Ibenk Trans (30% Gross)';
      } else if (currentAllReportCategory === 'sendiri') {
        ...
        if (lblPeng) lblPeng.innerText = 'Kas Bersih Ibenk Trans (100%)';
      } else {
        ...
        if (lblPeng) lblPeng.innerText = 'Hak Bersih Ibenk Trans';
      }
          ...
          if (currentAllReportCategory === 'sendiri') {
            thead.innerHTML = `
              <tr class="bg-gray-100 text-gray-700 font-bold border-y border-gray-300">
                ...
                <th class="p-2.5 text-right">Laba Bersih Kas Ibenk Trans (100%)</th>
              </tr>
            `;
            ...
            thead.innerHTML = `
              <tr class="bg-gray-100 text-gray-700 font-bold border-y border-gray-300">
                ...
                <th class="p-2.5 text-right">Biaya Ops</th>
                <th class="p-2.5 text-right">Porsi Ibenk Trans (30%)</th>
            ...
            thead.innerHTML = `
              <tr class="bg-gray-100 text-gray-700 font-bold border-y border-gray-300">
                ...
                <th class="p-2.5 text-right">Porsi Ibenk Trans</th>
            ...
            if (currentAllReportCategory === 'sendiri') {
              ...
              tfoot.innerHTML = `
                <tr class="bg-gray-100/95 text-gray-900 border-t-2 border-gray-400 text-xs">
                  ...
                  <td class="p-3 text-right font-black text-custom-green text-sm bg-green-50/70 border-x border-green-200">
                    ${formatRp(summary.total_porsi_pengelola || 0)}
                    <br><span class="text-[9px] text-custom-green font-normal">Kas Bersih Ibenk Trans</span>
                  </td>
                  <td class="p-3 text-right font-black text-purple-900 text-sm bg-purple-50/70">
                    ${formatRp(summary.total_porsi_investor || 0)}
                    <br><span class="text-[9px] text-purple-700 font-normal">Hak Semua Investor</span>
                      ...
                      <div>
                        ...
                        <p class="text-[10px] text-gray-500 font-medium">
                          ...
                          <span class="font-bold ${isInv ? 'text-purple-700' : 'text-custom-green'}">
                            ${isInv ? 'Mitra Investor (Bagi Hasil 70:30)' : 'Milik Ibenk Trans (100% Hak Kas)'}
                          </span>
                        </p>
                      </div>
                    </div>
                      ...
                      <thead>
                        <tr class="bg-gray-50 text-gray-600 font-bold border-b border-gray-200">
                          ...
                          <th class="p-2.5 text-right w-28 ${isInv ? 'text-blue-700' : 'text-custom-green'} font-bold">
                            ${isInv ? 'Porsi Ibenk (30%)' : 'Kas Ibenk (100%)'}
                          </th>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                      ...
                      <div class="bg-white p-2.5 rounded-lg border border-gray-200 shadow-2xs">
                        <span class="text-gray-500 font-semibold block text-[10px] uppercase">
                          ${isInv ? 'Kas Ibenk Trans (30% Gross)' : 'Kas Bersih Ibenk Trans (100%)'}
                        </span>
                      ...
                      <div class="${isInv ? 'bg-purple-50/80 border-purple-200' : 'bg-gray-50 border-gray-200'} p-2.5 rounded-lg border shadow-2xs">
                        ...
                        <span class="text-[9px] ${isInv ? 'text-purple-700' : 'text-gray-400'} block">
                          ${isInv ? '70% Gross - Biaya Operasional' : '100% Hak Kas Milik Ibenk Trans'}
                        </span>
                      </div>
                    </div>
                  </div>
    ...
    function downloadPDFAllReport() {
      ...
      const categoryNames = {
        all: 'Laporan_Keseluruhan_IbenkTrans',
        investor: 'Laporan_Unit_Investor_IbenkTrans',
        sendiri: 'Laporan_Unit_Milik_Sendiri_IbenkTrans'
      };
      let prefix = categoryNames[currentAllReportCategory] || 'Laporan_IbenkTrans';
      const mobilSelect = document.getElementById('allreport-filter-mobil');
      const mobilId = mobilSelect ? (mobilSelect.value || 'all') : 'all';
      if (mobilId !== 'all' && mobilSelect) {
