...
const SECRET_KEY = process.env.JWT_SECRET || 'secret_key_ibenktrans_majalengka';
    ...
    for (const ym of pastMonths) {
      ...
      const rincian_unit = allMobil.map(m => {
        ...
        return {
          ...
          kepemilikan: isInv ? 'Investor' : 'Ibenk Trans',
    ...
    targetMobil.forEach(m => {
      ...
      if (tx.length > 0 || (mobil_id && mobil_id !== 'all')) {
        ...
        detail_unit.push({
          ...
          plat_nomor: m.plat_nomor,
          tahun: m.tahun,
          kepemilikan: isInv ? 'Investor' : 'Ibenk Trans (Sendiri)',
          is_investor: isInv,
          total_transaksi: tx.length,
          total_pendapatan: pend,
          biaya_bbm: bbm,
...
if (!process.env.VERCEL) {
  app.listen(PORT, () => {
    console.log(`Server Ibenk Trans Berjalan di http://localhost:${PORT}`);
